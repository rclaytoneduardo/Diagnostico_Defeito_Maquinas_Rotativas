...
for disk_node = [11 13 15];
numsim=50;

a0=1.02*0.95; b0=1.02*1.05; r0 = a0 + (b0-a0).*rand(numsim,1); % unb mass
a1=0.7; b1=1.3; r1 = a1 + (b1-a1).*rand(numsim,1); % damping_factor variance
a2=0.8; b2=1.2; r2 = a2 + (b2-a2).*rand(numsim,1); % disk_thick variance
a3=0.8; b3=1.2; r3 = a3 + (b3-a3).*rand(numsim,1); % Bearing_c variance
...
Bearing_c = (15.958e-3-shaft_od)/2*r3(i) %valor verdadeiro = 5.23e-05 a 6.4e-5
...
uxmax=0;
uxend=0;
model.rubforce = [11 1000 0.6e5 0.2];
for ii=1:2
a4=0.30*(uxmax-uxend)+uxend; b4=0.50*(uxmax-uxend)+uxend; r4 = a4 + (b4-a4)*rand(1,1); % Rub
r4p=(r4-uxend)/(uxmax-uxend);
if r4>0
model.rubforce = [11 r4 0.6e5 0.2]; %[Node Clearance statot_stiffness frictional_coefficient]
end
...
if ii==1
x_dof_resp = 4*(model.rubforce(1))-3;
ux = response(x_dof_resp,:);
uxmax=max(ux);
uxend=max(ux(end-182*3:end));
else
...
%  Matrix of Feature 
databuilder(fail,matrix,disk_node,r0(i),r1(i),r2(i),r3(i),r4,r4p);% r4:folga[m], r4p: r4:folga[%] (% entre valor max e valor final)
